...
window.onload = function () {
    ...
    function _normalizeToComponents(url){
        if (!url) return url;
        if (
            url.startsWith('header_source/')  ||
            url.startsWith('section_source/') ||
            url.startsWith('footer_source/')  ||
            url.startsWith('slide_source/')   || // 혹시 빠져있다면
            url.startsWith('hero_source/')    ||
            url.startsWith('login_source/')
        ){
    ...
    function _ensureJs(src, cb) {
        ...
        if (window._dunopiLoaded.js.has(src)) {
            cb && cb();
            return;
        }
        ...
        s.src = src;
        s.onload = function () {
            window._dunopiLoaded.js.add(src);
            cb && cb();
        };
        s.onerror = function () {
            console.warn("[dunopi] script load failed:", src);
            cb && cb();
        };
    ...
    function injectComponentWithMeta(raw, componentBaseDir) {
        ...
        body.querySelectorAll('link[rel="stylesheet"][href]').forEach((el) =>
            cssHrefs.push(el.getAttribute("href"))
        );

        // 인라인 <style>
        let inlineCss = [];
        (head ? head.querySelectorAll("style") : []).forEach((el) =>
            inlineCss.push(el.textContent || "")
        );
        ...
        // 외부 JS
        let jsSrcs = [];
        (head ? head.querySelectorAll("script[src]") : []).forEach((el) =>
            jsSrcs.push(el.getAttribute("src"))
        );
        ...
        // 인라인 <script>
        let inlineJs = [];
        (head ? head.querySelectorAll("script:not([src])") : []).forEach((el) =>
            inlineJs.push(el.textContent || "")
        );
        body.querySelectorAll("script:not([src])").forEach((el) =>
            inlineJs.push(el.textContent || "")
        );
        ...
        const $wrapped = $(
            '<div class="dunopi_zone"><span class="btn_del_dunopi">삭제</span><i></i></div>'
        );
    ...
    function loadComponent(category, orderIndex) {
        var map = {
            ...
            hero   : { root: 'components/hero_source',    prefix: 'hero'    },
            login  : { root: 'components/login_source',   prefix: 'login'   }
        };
        ...
        function tryGet(paths, ok, fail, i) {
            i = i || 0;
            if (i >= paths.length) return fail();
            $.get(paths[i], ok).fail(function () {
                tryGet(paths, ok, fail, i + 1);
            });
        }
    }
    ...
    const _sameOrigin = (u) =>
        _isHttp(u) ? u.startsWith(location.origin) : !u.startsWith("/");
    ...
    // ZIP에 파일 추가하고 경로 반환
    async function _zipAddBinary(zip, path, url) {
        const buf = await _getArrayBuffer(url);
        zip.file(path, buf);
        return path;
    }
    ...
    async function _rewriteCssAndCollect(zip, cssUrl, cssText) {
        ...
        while ((m = re.exec(cssText)) !== null) {
            ...
            if (/^(data:|about:|chrome:)/i.test(raw)) {
                continue;
            }
    ...
    $(".view_wide_small").click(function () {
        ...
        if (dunopiH3 < 0.8) {
            $(".design_preview").css({
                transform:
                    "translate(-50%,0) scale(" + (dunopiH3 - 0.005) + ")",
            });
            setTimeout(function () {
                $(".design_preview .btn_del_dunopi").css({
                    transform: "scale(" + (1 - dunopiH3) / dunopiH3 + ")",
                    transformOrigin: "left",
                });
                $(".design_preview .btn_del_dunopi ~ i").css({
                    transform: "scaleX(" + (1 - dunopiH3) / dunopiH3 + ")",
                    transformOrigin: "left",
                });
            }, 500);
        } else {
    ...
    $(".view_wide_big").click(function () {
        ...
        $("body").append(
            '<div class="previewAll"><span class="btn_preview_close">전체보기 끄기</span>' +
                previewAllSource +
                "</div>"
        );
    ...
    $(".code_link a").click(function () {
        ...
        if (codeData != "all") {
            ...
            $(".code_box > div." + codeData + "_code").css({
                flexGrow: 1,
                padding: "10px",
            });
        } else {
    ...
    $("a.reset").click(function () {
        ...
        $(".content_zone blockquote").html(
            "일을 줄이자!!<span>FREEDOME</span>"
        );
    });
    ...
    function loginBox() {
        $("header#dunopi_main .login").click(function () {
            ...
            $("header#dunopi_main .login .login_cont")
                .toggle(300)
                .toggleClass("on");
        });
    }
